...
from loss.loss3d import loss_mpjpe, n_mpjpe, loss_velocity, loss_limb_var, loss_limb_gt, loss_angle, \
    loss_angle_velocity, loss_centroid
from loss.loss3d import jpe as calculate_jpe
from loss.loss3d import p_mpjpe as calculate_p_mpjpe
from loss.loss3d import mpjpe as calculate_mpjpe
from loss.loss3d import acc_error as calculate_acc_err
from loss.loss3d import centroid_error as calculate_centroid_err
...
def parse_args():
    ...
    parser.add_argument("--config", type=str, default="configs/model3d.yaml", help="Path to the config file.")
    parser.add_argument('-c', '--checkpoint', type=str, metavar='PATH',
                        help='checkpoint directory')
    parser.add_argument('--new-checkpoint', type=str, metavar='PATH', default='checkpoint1',
...
def train_one_epoch(args, model, train_loader, optimizer, device, losses):
    ...
    for x, y in tqdm(train_loader):
        ...
        # print(f"x:{x[0]};y{y[0]};pred{pred[0]}")#已normalized，[-1,1]之间
        optimizer.zero_grad()
        ...
        losses['centroid'].update(loss_ct.item(), batch_size)
...
def evaluate(args, model, test_loader, datareader, device):
    ...
    results_all = []
    centroid_errors_all = []  # Initialize list to store centroid errors
    model.eval()
    with torch.no_grad():
    ...
    # print("before:", results_all[0:2, ...])
    results_all = datareader.denormalize(results_all)
    # print("after_denormalize:", results_all[0:2, ...])
    _, split_id_test = datareader.get_split_id()
    actions = np.array(datareader.dt_dataset['test']['action'])#566920 ndarray
    ...
    center_err_all = np.zeros(num_test_frames)
    ...
    for idx in range(len(action_clips)):
        ...
        centroid_pred = np.mean(pred, axis=1)
        centroid_gt = np.mean(gt, axis=1)
        centroid_error = np.linalg.norm(centroid_pred - centroid_gt, axis=-1)
        centroid_errors_all.append(centroid_error)
        ...
        center_err = calculate_centroid_err(pred, gt)

        acc_err_all[frame_list[:-2]] += acc_err
        e1_all[frame_list] += err1
        err2 = calculate_p_mpjpe(pred, gt)
        e2_all[frame_list] += err2
        center_err_all[frame_list] += center_err
        oc[frame_list] += 1

    centroid_errors_all = np.concatenate(centroid_errors_all)
    ...
    ## final_result(_procrutes)内为15种动作的误差
    actions_errs1={}
    actions_errs2={}
    for i,act in enumerate(action_names):
        actions_errs1[act] = final_result[i]
        actions_errs2[str(act+'-p')]=final_result_procrustes[i]

    # Calculate centroid error
    centroid_error = np.mean(centroid_errors_all)

    print('Protocol #1 Error (MPJPE):', e1, 'mm')
    print('Acceleration error:', acceleration_error, 'mm/s^2')
    print('Protocol #2 Error (P-MPJPE):', e2, 'mm')
    print('Centroid error:', centroid_error, 'mm')  # Print centroid error
    print('----------')
    return e1, e2, joint_errors, acceleration_error, centroid_error, actions_errs1, actions_errs2
...
def train(args, opts):
    ...
    datareader = DataReaderH36M(n_frames=args.n_frames, sample_stride=1,
                                ...
                                dt_root='PoseDetector3d/data/motion3d', dt_file=args.dt_file)  # Used for H36m evaluation
    ...
    checkpoint_path_latest = os.path.join(opts.new_checkpoint, 'TA_latest_epoch.pth.tr')
    checkpoint_path_best = os.path.join(opts.new_checkpoint, 'TA_best_epoch.pth.tr')

    for epoch in range(epoch_start, args.epochs):
        if opts.eval_only:
            e1, e2, joint_errors, acceleration_error, centroid_error, actions_errs1, actions_errs2=evaluate(args, model, test_loader, datareader, device)
            
            print(actions_errs1,actions_errs2)
            sys.exit()
        ...
        mpjpe, p_mpjpe, joints_error, acceleration_error, centroid_error, actions_errs1, actions_errs2 = evaluate(args, model, test_loader,
                                                                                    datareader, device)

        if mpjpe < min_mpjpe:
            min_mpjpe = mpjpe
            save_checkpoint(checkpoint_path_best, epoch, lr, optimizer, model, min_mpjpe, wandb_id)
            print("mpjpe,p_mpjpe,acceleration_error,centroid_error:", mpjpe, p_mpjpe, acceleration_error,
                  centroid_error)
...
def test():
    ...
    datareader = DataReaderH36M(n_frames=args.n_frames, sample_stride=1,
                                data_stride_train=args.n_frames // 3, data_stride_test=args.n_frames,
                                dt_root='PoseDetector3d/data/motion3d', dt_file=args.dt_file)
