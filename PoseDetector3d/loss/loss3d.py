...
def centroid_error(predicted, target):
    """
    计算预测和目标之间的质心误差。
    :param predicted: NumPy数组，形状为(N, T, 17, 3)
    :param target: NumPy数组，形状为(N, T, 17, 3)
    :return: 平均质心误差
    """
    assert predicted.shape == target.shape
    centroid_predicted = np.mean(predicted, axis=-2)
    centroid_target = np.mean(target, axis=-2)
    return np.mean(np.linalg.norm(centroid_predicted - centroid_target, axis=-1))


def loss_centroid(predicted, target):
    '''
    center of the body in each frame
    :param predicted:
    :param target:
    :return:
    '''
    assert predicted.shape == target.shape
    centroid_predicted = torch.mean(predicted, dim=-2)
    centroid_target = torch.mean(target, dim=-2)
    return torch.mean(torch.norm(centroid_predicted - centroid_target, dim=-1))
